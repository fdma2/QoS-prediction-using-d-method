function [userID,serviceID] = getSimilarUserAndService5(targetUser,targetService,rt,percent,top_num)
...
for i = 1:num_user
    ...
    for j = 1:num_service
        ...
        matrix_user_compare_value(i,j) = getDTW(temp_service_quality(tmp_1) , target_service_quality(tmp_2));
...
[ ~, best_user] = sort(user_similar_value);
userID = best_user;
...
for j = 1: num_service
    ...
    for i = 1: num_user
        ...
        tmp_2 = getFP(temp_service_quality_s2);
        matrix_service_compare_value(i,j) = getDTW(temp_service_quality(tmp_1) , target_service_quality(tmp_2));
...
[~ , best_service] = sort(service_similar_value);
serviceID = best_service;
%%====find similar service finish==============
fprintf('target:%d,	%d\n', targetUser,targetService);
fprintf('best  :%d, %d            compare:%d, %d\n', userID(1:top_num),serviceID(1:top_num),user_compared_num(userID(1:top_num)),service_compared_num(serviceID(1:top_num)));