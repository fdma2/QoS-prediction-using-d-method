...
for i = 1:num_user
    ...
    for j = 1:num_service
        if sum(abs(rt(i,j,1))) == 0 || sum(abs(rt(targetUser,j,1))) == 0  %if one of the two value compared is 0,   jump to next loop
            continue;
        end
...
for j = 1: num_service
    ...
    for i = 1: num_user
        if sum(abs(rt(i,j,1))) == 0 || sum(abs(rt(i,targetService,1))) == 0  %if one of the two value compared is 0,   jump to next loop
            continue;
        end
...
fprintf('best user: %d , compare items: %d , best service: %d , compare times:%d\n', userID,user_compared_num(userID),serviceID,service_compared_num(serviceID));