...
for i=1:k
    ...
    for j = 1:top_num
        user_top(:,j) = reshape(dataset(userID(j),targetService,:),[num,1]) - user_delta_cal(dataset,targetUser,targetService,userID(j));   %
        service_top(:,j) = reshape(dataset(targetUser,serviceID(j),:),[num,1])-service_delta_cal(dataset,targetUser,targetService,serviceID(j));
    ...
    realValue{i} = reshape(dataset(targetUser,targetService,:),[num,1]);     % 这里有错误，这是 1X1X64的矩阵, 已改为reshape
    %#################开始画图###################
    %用相似服务预测
